...
export class AdminTableService {
  ...
  getTable(columnName?: string, page?: number, size?: number, sortingType?: string, orderStatus?: string[]) {
    return this.http.get<any[]>(
      `${this.url}bigOrderTable?sortBy=${columnName}&pageNumber=${page}&pageSize=${size}&sortDirection=${sortingType}&orderStatus=${orderStatus}`
    );
  }
