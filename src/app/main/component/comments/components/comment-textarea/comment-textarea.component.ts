...
export class CommentTextareaComponent implements OnInit, AfterViewInit, OnChanges, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.socketServiceSubscription = this.SocketServ.onMessage(`/topic/${this.userId}/searchUsers`).subscribe((data: TaggedUser[]) => {
      if (data.length) {
  ...
  sendSocketMessage(query: string): void {
    this.SocketServ.send('/app/getUsersToTagInComment', { currentUserId: this.userId, searchQuery: query });
  }
