...
describe("EcoNewsDetailComponent", () => {
  ...
  it("ngOnDestroy should destroy two method ", () => {
    ...
    expect(
      (component as any).newsIdSubscription.unsubscribe
    ).toHaveBeenCalledTimes(1);
    expect(
      (component as any).newsItemSubscription.unsubscribe
    ).toHaveBeenCalledTimes(1);
  });
  ...
  it("setNewsItem should compare edited item with EcoNewsModel interface", () => {
    ...
    expect(component.newsItem).toEqual({
      ...mockEcoNewsModel,
      ...nestedNewsItem,
    });
  });
  ...
  it("fetchNewsItem should return item by id", async () => {
    ...
    (component as any).newsItemSubscription = ecoNewsService
      .getEcoNewsById(id)
    ...
    const request = httpMock.expectOne(
      `https://greencity.azurewebsites.net/econews/${id}`
    );
